...
\hypersetup{
	pdftitle= {Diseño y desarrollo de un microservicio para la gestión de información de monitorización y predicciones de tráfico en red},
	pdfauthor = {Enrique Fernández Sánchez},
	pdfsubject = {Aplicación de gestión de datos de monitorización y predicción de tráfico en red},
	pdfkeywords = {microservice, monitoring, traffic forecast, Python, FastAPI, network forecast}
}
...
\begin{document}
	...
	\noindent Entendemos predicción de tráfico de red como la estimación del tráfico que existirá en un instante futuro en dicha red, en función del histórico de tráfico de ese sistema. En función de la escala de tiempo sobre que la que trabajemos, podemos tener diferentes casos de uso: [\ref{bib: apuntes OeIR}]
	
	\begin{itemize}
		\item Escala de \textbf{minutos}. Detectar anomalías de tráfico. Anticipar picos de tráfico y situaciones de congestión.
		
		\item Escala de \textbf{horas}. Adaptar el encaminamiento en función del tráfico.
		
		\item Escala de \textbf{meses}. Permite planificar las capacidades de la red a largo plazo.
	\end{itemize}
	...
	\begin{itemize}
		...
		\item Permitir la ejecución de predicciones de tráfico. Se desea poder elegir los intervalos de tiempo a predecir, de modo que se puedan realizar predicciones tanto en corto como a largo plazo.
	...
	\begin{itemize}
		\item API de SOAP. [\ref{bib: soap}] Protocolo tradicional de servicios web. Se basa en acceso a objetos mediante mensajes XML, encapsulados en el protocolo HTTP. Es necesario que ambos interlocutores conozcan la estructura de objetos del servicio web, y que ambos utilicen la misma versión. Normalmente se utilizan ficheros WSDL (Web Services Description Language) que permiten conocer el servicio y el conjunto de operaciones ofrecidas por este [\ref{bib: soap wsdl}]. El usuario depende de este archivo para conocer qué puedes hacer en la aplicación. En general, es una solución poco flexible, ya que es necesario conocer cómo esta implementado servicio para poder realizar peticiones, además de ser complejo de programar; es por esto por lo que se ha sustituido por otras implementaciones más flexibles y sencillas de integrar.
		...
		\item API de REST. Solución más popular. Es un servicio orientado a los recursos, el cliente envía peticiones al servidor utilizando métodos HTTP. Es una opción muy flexible, ya que son sencillos tanto de programar como de mantener, además de que escalan fácilmente.
	\end{itemize}
	...
	\begin{figure}[h!]
		\begin{center}
			\includegraphics[width=0.59\textwidth]{img/microservice_architecture.png}
	...
	\begin{itemize}
		\item Concebido para almacenar datos de telemetría, como métricas de aplicaciones o sensores IoT, lo que es idóneo para nuestros casos de uso de interés.
	...
	\begin{itemize}
		...
		\item Robusto: código pensado para entornos de producción, además de incluir documentación automática (usando Swagger UI [\ref{bib: swagger ui github}] o ReDoc [\ref{bib: redoc github}]).
	...
	\noindent Prophet está basado en inferencia estadística, lo que permite tener un rendimiento mayor al que tendríamos si utilizáramos técnicas de \textit{Machine Learning} para solucionar el mismo problema de predicción de datos. Además, es robusto a datos no disponibles y a modificaciones aleatorias sobre las tendencias. 
...
services:
  ...
  influxdb:
	...
	\begin{table}[h!]
		...
		\caption{CRUD \texttt{interfaces} (* equivale a \textit{acceso denegado})}
	...
	\begin{figure}[h!]
		\includegraphics[width=0.85\textwidth, center]{diag/traffic_forecast_schema.png}
	...
	\begin{enumerate}
        ...
        \textit{Apuntes de la asignatura \texttt{Operación e Ingeniería de Red}, del Máster Universitario en Ingeniería de Telecomunicaciones de la UPCT}: \url{https://teleco.upct.es/guia-docente/211101007}

		% 2
		\item
		\label{bib: what is api}
        \textit{¿Qué es una API?}: \url{https://aws.amazon.com/es/what-is/api/}

        % 3
		\item
		\label{bib: soap}
        \textit{SOAP (Simple Object Access Protocol)}: \url{https://www.f5.com/es_es/services/resources/glossary/simple-object-access-protocol-soap}

        % 4
		\item
		\label{bib: soap wsdl}
        \textit{WSDL (Web Services Description Language)}: \url{https://www.tutorialworks.com/wsdl/}
  
		% 5
		\item
		\label{bib: microservices}
        \textit{Microsoft Learn: Microservice architecture style}: \url{https://learn.microsoft.com/en-us/azure/architecture/guide/architecture-styles/microservices}
		
		% 6
		\item
		\label{bib: relational database}
        \textit{What is a relational database?}: \url{https://cloud.google.com/learn/what-is-a-relational-database}
				
		% 7
		\item
		\label{bib: postgresql}
        \textit{PostgreSQL}: \url{https://www.postgresql.org/}
		
		% 8
		\item
		\label{bib: influxdb doc}
        \textit{InfluxDB: Introduction}: \url{https://www.stackhero.io/en/services/InfluxDB/documentations/Introduction}
		
		% 9
		\item
		\label{bib: influxdb doc vs sql}
        \textit{InfluxDB: Comparison to SQL}: \url{https://docs.influxdata.com/enterprise_influxdb/v1.10/concepts/crosswalk/}
        ...
        \textit{FastAPI framework}: \url{https://fastapi.tiangolo.com/}

        % 12
		\item
		\label{bib: swagger ui github}
        \textit{GitHub: Swagger UI}: \url{https://github.com/swagger-api/swagger-ui}

        % 13
		\item
		\label{bib: redoc github}
        \textit{GitHub: ReDoc}: \url{https://github.com/Redocly/redoc}
		
		% 14
		\item
		\label{bib: openapi}
        \textit{GitHub: OpenAPI-Specification}: \url{https://github.com/OAI/OpenAPI-Specification}
		
		% 15
		\item
		\label{bib: json schema}
        \textit{JSON Schema}: \url{https://json-schema.org/}
		
		% 16
		\item
		\label{bib: prophet doc}
        \textit{Prophet}: \url{https://facebook.github.io/prophet/}
		
		% 17
		\item
		\label{bib: docker compose doc}
        \textit{Docker Compose Overview}: \url{https://docs.docker.com/compose/}
		
		% 18
		\item
		\label{bib: traefik webpage}
        \textit{Traefik Webpage}: \url{https://traefik.io/traefik/}
		
		% 19
		\item
		\label{bib: traefik wiki}
        \textit{Traefik Wiki}: \url{https://doc.traefik.io/traefik/}

        % 20
		\item
		\label{bib: factory method}
        \textit{Factory Method - Python Design Patterns}: \url{https://www.geeksforgeeks.org/factory-method-python-design-patterns/}
		
		% 21
		\item
		\label{bib: fastapi doc bigger apps}
        \textit{FastAPI Doc: Bigger Applications}: \url{https://fastapi.tiangolo.com/tutorial/bigger-applications/}
		
		% 22
		\item
		\label{bib: orm python}
        \textit{Object-relational Mappers (ORMs)}: \url{https://www.fullstackpython.com/object-relational-mappers-orms.html}
		
		% 23
		\item
		\label{bib: fastapi orm}
        \textit{FastAPI Doc: SQL (Relational) Databases. ORM}: \url{https://fastapi.tiangolo.com/tutorial/sql-databases/#orms}
		
		% 24
		\item
		\label{bib: fastapi migrations}
        \textit{FastAPI Doc: SQL (Relational) Databases. Migrations}: \url{https://fastapi.tiangolo.com/tutorial/sql-databases/#migrations}
		
		% 25
		\item
		\label{bib: prophet parameters}
        \textit{Prophet Doc: Hyperparameter-tuning}: \url{https://facebook.github.io/prophet/docs/diagnostics.html#hyperparameter-tuning}
		
		% 26
		\item
		\label{bib: prophet trend flex}
        \textit{Prophet Doc: Adjusting trend flexibility}: \url{https://facebook.github.io/prophet/docs/trend_changepoints.html#adjusting-trend-flexibility}
		
		% 27
		\item
		\label{bib: cisco white paper}
        \textit{Cisco Annual Internet Report (2018-2023) White paper}: \url{https://www.cisco.com/c/en/us/solutions/collateral/executive-perspectives/annual-internet-report/white-paper-c11-741490.html}
	...
	\begin{enumerate}
        ...
        \textit{Monolithic vs Microservices Architecture}: \url{https://xbsoftware.com/blog/microservices-vs-monolithic-architecture/}
		
		% 2
		\item
		\label{bib_img: example sql}
        \textit{Relational Databases}: \url{https://xbsoftware.com/blog/main-types-of-database-management-systems/}
		
		% 3
		\item
		\label{bib_img: influxdb logo}
        \textit{InfluxDB}: \url{https://www.influxdata.com/}
        ...
        \textit{FastAPI}: \url{https://fastapi.tiangolo.com/}
        ...
        \textit{Docker Docs: Get Started}: \url{https://docs.docker.com/get-started/overview/}
		
		% 8
		\item
		\label{bib_img: traefik capture}
        \textit{Traefik website}: \url{https://traefik.io/traefik/}
